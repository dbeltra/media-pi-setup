...
The gluetun service is configured with **`VPN_SERVICE_PROVIDER=custom`** and an explicit `WIREGUARD_ENDPOINT_IP` / `WIREGUARD_ENDPOINT_PORT` / `WIREGUARD_PUBLIC_KEY` (the `VPN_ENDPOINT_*` spellings are deprecated and log a warning on every start), rather than `nordvpn` + `SERVER_HOSTNAMES`. Reason: gluetun's *bundled* server database is stale in every release — v3.41.3 rejects `nl1020`, and even `latest` rejects `nl1252` — so pinning by hostname fails outright on any recent server. The custom endpoint bypasses the database entirely.
...
**LAN clients already use Pi-hole** — the router's DHCP pool for `192.168.1.33–199` hands out DNS `192.168.1.47` (Pi-hole) with `1.1.1.1` secondary. Verified 2026-08-30 from a LAN client: `doubleclick.net` → `0.0.0.0`, `github.com` resolves normally. Do not "fix" this by reading the DNS off DHCP pool index 0 — that pool serves the **IPTV decoders** and legitimately points at Movistar's own resolver.

**Keep the `1.1.1.1` secondary.** It does not cause a meaningful bypass: Pi-hole answers blocked domains authoritatively with `0.0.0.0`, which is a successful response, so clients never retry the secondary. What it does buy is outage insurance — during the 2026-08-30 IP conflict every device's primary DNS pointed at a phone squatting on `.47`, and `1.1.1.1` is the only reason the house kept resolving. Removing it would trade a marginal blocking gain for a house-wide single point of failure.
